...
def feature_vecs_NLP(train_pos, train_neg, test_pos, test_neg):
    ...
    pos_top_one=len(train_pos)*0.01
    neg_top_one=len(train_neg)*0.01
    
    for k,v in pos.items():
        if k not in stopwords:
            if v>=pos_top_one :
                count_neg=neg.get(k,0)
                #twice as many positive as neg OR twice as negative as positive
                if(v>=count_neg or count_neg>2*v):
                    pos_dict[k]=v
                    
    for k,v in neg.items():
        if k not in stopwords:
            if v>=neg_top_one :
                count_pos=pos.get(k,0)
                #twice as many negative as pos OR twice as pos as negative
                if(v>=count_pos or count_pos>2*v):
                    neg_dict[k]=v
                    
    # Using the above words as features, construct binary vectors for each text in the training and test set.
    features=list(set(list(pos_dict.keys())+ list(neg_dict.keys())))
    ...
    for l in train_pos:
        temp=dict.fromkeys(features, 0)
        for w in l:
            if w in temp:
                temp[w]=1
           
        train_pos_vec.append(temp.values())
    ...
    for l in train_neg:
        ...
        train_neg_vec.append(temp.values())
    ...
    for l in test_pos:
        ...
        test_pos_vec.append(temp.values())
    ...
    for l in train_neg:
        ...
        test_neg_vec.append(temp.values())
